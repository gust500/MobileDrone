...
//            return new FfmpegBytesInputsIOSMemory(inputOptions, command);
        ...
        public static FfmpegBytesInputs GetNewInstanceForFfplay()
        {
#if UNITY_EDITOR_WIN || UNITY_EDITOR_LINUX || UNITY_STANDALONE_WIN || UNITY_STANDALONE_LINUX
            return new FfmpegBytesInputsWinLib();
#elif UNITY_EDITOR_OSX || UNITY_STANDALONE_OSX
            return new FfmpegBytesInputsIOSMemory();
#elif UNITY_ANDROID
            return new FfmpegBytesInputsAndroidPipe();
#elif UNITY_IOS
            return new FfmpegBytesInputsIOSPipe();
#else
            return null;
#endif
        }
        ...
        public FfmpegBytesInputs()
        {
            inputOptions_ = new string[1];
            resetInputBytes();
        }
        ...
        protected virtual void DisposeThreads()
        {
            foreach (var thread in Threads)
            {
                thread.Join();
